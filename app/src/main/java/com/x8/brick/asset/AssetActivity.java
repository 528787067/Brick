...
import com.x8.brick.okhttp3.executor.android.AssetAsyncTaskExecutorFactory;
import com.x8.brick.task.Task;

import java.io.IOException;

public class AssetActivity extends AppCompatActivity implements View.OnClickListener, Task.Callback<HttpResponse> {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
         ...
         * AssetAsyncTaskExecutorFactory 执行器工厂使用 AssetAsyncTaskExecutor 执行器进行处理
         * AssetAsyncTaskExecutor 执行器会根据请求的 URL 的相对地址读取 asset 目录下的对应文件来模拟网络请求
         * AssetAsyncTaskExecutor 执行器使用的是 AsyncTask 进行异步处理，因此可以直接回调到主线程
         */
        HttpClient httpClient = new HttpClient.Builder()
                .setExecutorFactory(new AssetAsyncTaskExecutorFactory(this))
                .build();
        HttpManager httpManager = new HttpManager.Builder(httpClient).build();
        api = httpManager.create(AssetApi.class);
    }
    ...
    private void getUser() {
        HttpTask<HttpResponse> task = api.getUser("李小二", 17);
    ...
    private void postUser() {
        HttpTask<HttpResponse> task = api.postUser("王大锤", 23);
        task.asyncExecute(this);
    }
