...
public class GsonActivity extends AppCompatActivity implements Task.Callback<ResponseBean<UserBean>> {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        // Gson 转换器可以实现将 HttpResponse 数据直接转换成实体对象
        HttpClient httpClient = new HttpClient.Builder().build();
        HttpManager httpManager = new HttpManager.Builder(httpClient)
                .addResponseConverter(new HttpGsonResponseConverter<>()) // 添加 Gson 转换器
                .build();
        final GsonApi api = httpManager.create(GsonApi.class);

        findViewById(R.id.get_data).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                HttpTask<ResponseBean<UserBean>> task = api.getUser("江小白", 26);
